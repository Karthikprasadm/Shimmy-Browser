...
function fromEnvelope(envelope: ChatErrorEnvelope): ChatErrorView {
  ...
  return {
    ...
    isRateLimit: envelope.code === 'rate_limited',
    isCreditsExhausted: envelope.code === 'credits_exhausted',
    isConnectionError: envelope.code === 'connection_failed',
  }
}
...
export const ChatError: FC<ChatErrorProps> = ({
  ...
  useEffect(() => {
    if (!view.isCreditsExhausted) return
    markCreditsExhausted()
    invalidateCredits()
  }, [invalidateCredits, view.isCreditsExhausted, markCreditsExhausted])
    ...
    <div className="mx-4 flex flex-col items-center justify-center gap-3 rounded-lg border border-destructive/30 bg-destructive/5 p-4">
      <div className="flex items-center gap-2 text-muted-foreground">
        ...
        <span className="font-medium text-sm">
          {view.isUpstreamRateLimit && view.providerName && view.providerName !== 'your provider'
            ? `${view.providerName} rate limit reached`
            : view.title}
        </span>
      </div>
      <p className="text-center text-destructive text-xs">{view.text}</p>
      {view.isUpstreamRateLimit && (
